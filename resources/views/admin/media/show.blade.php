                    ...
                    <div class="card-body">
                        ...
                        <div class="card-header row mb-3">
                            <div class="col-md-6 text-center">
                                <img src="{{ getFileFromStorage($media->getCoverImage(), 'storage') }}" alt="Cover Image"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body row">
                        <video class="col-12" width="100%" height="320" controls>
                            <source src="{{ route('watch_video_attachment', encrypt($media->getAttachment())) }}"
                                type="video/mp4">
        ...
        <div class="modal-dialog modal-dialog-centered modal-md" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" action="{{ route('media.update', $media->id) }}" enctype="multipart/form-data">@csrf
                        @method('put')
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="level" style="height:45px" aria-required="true">
                                        ...
                                        @foreach ($levels as $key => $value)
                                            <option value="{{ $key }}" {{ $media->level == $key ? 'selected' : '' }}>
                                                {{ $value }}</option>
                                        @endforeach
                                    </select>
                                    @error('level')
                                    ...
                                    <select class="form-control" name="klass_id" style="height:45px" aria-required="true">
                                        ...
                                        @foreach ($klasses as $klass)
                                            <option value="{{ $klass->id }}"
                                                {{ $media->klass_id == $klass->id ? 'selected' : '' }}>{{ $klass->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('klass_id')
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="term" style="height:45px" aria-required="true">
                                        ...
                                        @foreach ($terms as $key => $value)
                                            <option value="{{ $key }}" {{ $media->term == $key ? 'selected' : '' }}>
                                                {{ $value }}</option>
                                        @endforeach
                                    </select>
                                    @error('term')
                                    ...
                                    <select class="form-control" name="subject_id" style="height:45px" aria-required="true">
                                        ...
                                        @foreach ($subjects as $subject)
                                            <option value="{{ $subject->id }}"
                                                {{ $media->subject_id == $subject->id ? 'selected' : '' }}>
                                                {{ $subject->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('subject_id')
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="status" style="height:45px" aria-required="true">
                                        <option disabled selected>Select One</option>
                                        <option value="Visible" {{ $media->status == 'Visible' ? 'selected' : '' }}>Visible
                                        </option>
                                        <option value="Hidden" {{ $media->status == 'Hidden' ? 'selected' : '' }}>Hidden
                                        </option>
                                    </select>
                                </div>
